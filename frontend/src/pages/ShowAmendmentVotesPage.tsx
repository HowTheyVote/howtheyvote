...
export const ShowAmendmentVotesPage: Page<Vote> = ({ data }) => {
  return (
    <App
      title={[data.display_title, "Amendments"]}
    >
      <BaseLayout>
        <Stack space="lg">
          ...
          <Wrapper>
            <Callout
            title="Hello">
              <p>The votes are <b>amendments</b>. Amendments change the final text that parliament votes on, but do not produce Parliament decisions on their own. You can see the result of the final vote <a href={`/votes/${data.id}`}>here</a></p>
            </Callout>
            <div className="px">
              <AmendmentVoteCards
                votes={data.related}
