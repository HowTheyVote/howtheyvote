...
def member_has_term(term: int) -> Exists:
    ...
    exp = func.json_each(Member.terms).table_valued("value")  # type: ignore[no-untyped-call]
...
def member_active_at(date: datetime.date) -> Exists:
    ...
    exp = func.json_each(Member.group_memberships).table_valued("value")  # type: ignore[no-untyped-call]
...
def press_release_references_vote(vote: Vote) -> ColumnElement[bool]:
    ...
    report/resolution."""
    ref_exp = func.json_each(PressRelease.references).table_valued("value")  # type: ignore[no-untyped-call]
    proc_ref_exp = func.json_each(PressRelease.procedure_references).table_valued("value")  # type: ignore[no-untyped-call]
